...
pub fn build_flash_attn_prefill(head_dim: u32, gqa_ratio_log2: u8) -> BlockKernel {
    let wg_size = head_dim;
    let mut kb = BlockKernel::new("flash_attn_prefill", wg_size);
    ...
    let kv_stride = kb.arg_u32("kv_stride");
    let n_heads = kb.arg_u32("n_heads");
    let q_stride = kb.arg_u32("q_stride");
    let pos = kb.arg_u32("pos");

    let tid = kb.arange(0, wg_size);
    let head = kb.program_id(0);
    let row = kb.program_id(1);

    let kv_h = head.shr(&mut kb, gqa_ratio_log2);
    let kv_h_offset = kv_h.mul(&mut kb, head_dim_arg);
    let h_offset = head.mul(&mut kb, head_dim_arg);
    ...
    let q_row_offset = row.mul(&mut kb, q_stride);
    let q_offset = q_row_offset.add(&mut kb, h_offset).add(&mut kb, tid);
    ...
    // Causal mask: query row r attends to KV positions 0..(pos+r)
    let causal_end = pos.add(&mut kb, row).add(&mut kb, one_u);
    let iter = kb.for_range(zero_u, causal_end, 1);
    {
        let old_max = kb.lds_load(lds_base, offset_max);
        let old_sum = kb.lds_load(lds_base, offset_sum);
        let old_acc = kb.lds_load(lds_base, tid);
    ...
    let out_row_offset = row.mul(&mut kb, q_stride);
    let out_offset = out_row_offset.add(&mut kb, h_offset).add(&mut kb, tid);
    kb.store(out_ptr, out_offset, result, d_mask);

    kb
}

pub fn flash_attn_prefill_grid(n_heads: u32, head_dim: u32, seq_len: u32) -> (u32, u32) {
    (n_heads * head_dim, seq_len)
}

pub fn cpu_flash_attn_prefill(
    q: &[f32],
    k: &[f32],
    v: &[f32],
    n_heads: usize,
    ...
    for h in 0..n_heads {
        ...
        for r in 0..seq_len {
            let q_base = r * q_stride + h * head_dim;
            let kv_base = kv_h * head_dim;
            let causal_end = (pos + r + 1).min(kv_len);

            let mut max_val = f32::NEG_INFINITY;
            let mut sum_exp = 0.0f32;
            let mut acc = vec![0.0f32; head_dim];

            for kv in 0..causal_end {
                let mut dot = 0.0f32;
                for d in 0..head_dim {
            ...
            let out_base = r * q_stride + h * head_dim;
            for d in 0..head_dim {
                out[out_base + d] = acc[d] / sum_exp;
            }
        }
    }
    out
...
mod tests {
    ...
    #[test]
    fn test_flash_attn_prefill_compiles() {
        let kb = build_flash_attn_prefill(128, 1);
        let ck = kb.compile_via_ssa(Target::GFX1100).expect("flash_attn_prefill compile");
        assert!(!ck.elf.is_empty());
        eprintln!("flash_attn_prefill: {} bytes ELF, wg={:?}, lds={}",
            ck.elf.len(), ck.workgroup_size, ck.lds_size);
    }

    #[test]
    fn test_cpu_flash_prefill_matches_standard() {
        let head_dim = 8;
        let n_heads = 4;
        let n_kv_heads = 2;
        let seq_len = 3;
        ...
        let q: Vec<f32> = (0..seq_len * n_heads * head_dim).map(|_| rand()).collect();
        let k: Vec<f32> = (0..kv_len * n_kv_heads * head_dim).map(|_| rand()).collect();
        let v: Vec<f32> = (0..kv_len * n_kv_heads * head_dim).map(|_| rand()).collect();

        let flash_out = cpu_flash_attn_prefill(
            &q, &k, &v, n_heads, n_kv_heads, head_dim, kv_len, seq_len, pos,
        );

        let gqa_ratio = n_heads / n_kv_heads;
        let q_stride = n_heads * head_dim;
        let kv_stride = n_kv_heads * head_dim;
        let mut std_out = vec![0.0f32; seq_len * n_heads * head_dim];

        for h in 0..n_heads {
            let kv_h = h / gqa_ratio;
            for r in 0..seq_len {
                let causal_end = (pos + r + 1).min(kv_len);
                // scores = Q[row, h, :] @ K[0..causal_end, kv_h, :]^T
                let mut scores = vec![0.0f32; causal_end];
                for kv in 0..causal_end {
                    let mut dot = 0.0f32;
                    for d in 0..head_dim {
                        dot += q[r * q_stride + h * head_dim + d]
                             * k[kv * kv_stride + kv_h * head_dim + d];
                    }
                    scores[kv] = dot;
                }
                // softmax with scale
                let scale = 1.0 / (head_dim as f32).sqrt();
                let max_val = scores.iter().map(|s| s * scale).fold(f32::NEG_INFINITY, f32::max);
                let mut weights = vec![0.0f32; causal_end];
                let mut sum = 0.0f32;
                for kv in 0..causal_end {
                    weights[kv] = (scores[kv] * scale - max_val).exp();
                    sum += weights[kv];
                }
                for kv in 0..causal_end {
                    weights[kv] /= sum;
                ...
                for d in 0..head_dim {
                    let mut val = 0.0f32;
                    for kv in 0..causal_end {
                        val += weights[kv] * v[kv * kv_stride + kv_h * head_dim + d];
                    }
                    std_out[r * q_stride + h * head_dim + d] = val;
                }
            }
        }
        ...
        assert!(max_err < 1e-5,
            "CPU flash prefill vs standard: max_err={}", max_err);
    }
}
